...
#include "SlipWeakeningFrictionczm3dCDBMOverstress.h"
#include "InterfaceKernel.h"

registerMooseObject("DynamicCDBMApp", SlipWeakeningFrictionczm3dCDBMOverstress);

InputParameters
SlipWeakeningFrictionczm3dCDBMOverstress::validParams()
{
...
SlipWeakeningFrictionczm3dCDBMOverstress::SlipWeakeningFrictionczm3dCDBMOverstress(const InputParameters & parameters)
  ...
  // Only retrieve the cohesion function name when cohesion is enabled; otherwise, use a default
  // constructed FunctionName to avoid accessing an unset parameter.
  _cohesion_function_name(_use_cohesion ? getParam<FunctionName>("cohesion_function") : FunctionName()),
  // Initialize pointer to nullptr; it will only be set if cohesion is enabled
  _cohesion_function(nullptr)    
{
...
void
SlipWeakeningFrictionczm3dCDBMOverstress::computeInterfaceTractionAndDerivatives()
{
