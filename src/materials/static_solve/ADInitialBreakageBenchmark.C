...
#include "ADInitialBreakageBenchmark.h"

/**
 *  Created by Chunhui Zhao, Dec 4th, 2024
 *  ADMaterial used in defining initial breakage profile with exponential decay along the normal direction
 */
registerMooseObject("DynamicCDBMApp", ADInitialBreakageBenchmark);

InputParameters
ADInitialBreakageBenchmark::validParams()
{
  ...
  params.addRequiredParam<Real>("nucl_breakage","the damage value apply within the nucleation region");
  params.addRequiredParam<Real>("e_breakage","the peak damage value apply region normal to the fault (exponential decay)");
...
ADInitialBreakageBenchmark::ADInitialBreakageBenchmark(const InputParameters & parameters)
  : ADMaterial(parameters),
  _initial_damage(declareADProperty<Real>("initial_breakage")),
  ...
  _nucl_damage(getParam<Real>("nucl_breakage")),
  _peak_damage(getParam<Real>("e_breakage")),
...
void
ADInitialBreakageBenchmark::initQpStatefulProperties()
{
}

void
ADInitialBreakageBenchmark::computeQpProperties()
{
