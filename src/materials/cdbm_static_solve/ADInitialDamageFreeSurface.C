...
#include "ADInitialDamageFreeSurface.h"

/**
 *  Created by Chunhui Zhao, Jul 18th, 2024
 *  ADMaterial used in defining initial damage profile
 */
registerMooseObject("DynamicCDBMApp", ADInitialDamageFreeSurface);

InputParameters
ADInitialDamageFreeSurface::validParams()
{
...
ADInitialDamageFreeSurface::ADInitialDamageFreeSurface(const InputParameters & parameters)
...
void
ADInitialDamageFreeSurface::initQpStatefulProperties()
{
  ...
  Real alpha_o = 0.0;
  if (xcoord >= -30 && xcoord <= 30 && ycoord >= 240 && ycoord <= 250 && zcoord >= -0.5 && zcoord <= 0.5){
    if ( xcoord >= -26 && xcoord <= -22 && ycoord >= 243 && ycoord <= 247 ){
...
void
ADInitialDamageFreeSurface::computeQpProperties()
{
