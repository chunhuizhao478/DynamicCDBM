...
registerMooseObject("DynamicCDBMApp", ComputeDamageBreakageStress3DSlipWeakening);
...
ComputeDamageBreakageStress3DSlipWeakening::computeStrainRateCd()
{
  ...
  if (_deviatroic_strain_rate_old[_qp] < _strain_rate_hat){
    _Cd_mat[_qp] = _cd_hat; //if deviatoric strain rate is less than strain_rate_hat, Cd = 0
  }
  else{
