...
#include "FarmsNodePositions.h"

#include "libmesh/parallel_algebra.h"

registerMooseObject("DynamicCDBMApp", FarmsNodePositions);

InputParameters
FarmsNodePositions::validParams()
{
...
FarmsNodePositions::FarmsNodePositions(const InputParameters & parameters)
...
FarmsNodePositions::initialize()
{
  ...
  {
    const auto node = _mesh.queryNodePtr(node_id);
    if (!node)
      continue;
    ...
    if (binfo)
    {
      ...
      {
        const auto iter_pr = binfo->get_nodeset_map().equal_range(node);
        for (auto nid = iter_pr.first; nid != binfo->get_nodeset_map().equal_range(node).second;
             nid++)
          if (hasBoundary(nid->second))
            goto inRestriction;
      }
      continue;
    }
